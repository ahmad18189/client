...
def edit_allocation_new_balance():
    ...
    with open('/home/frappe/frappe-bench/apps/client/client/final_new_leave_balance.csv', "r") as infile:  
        rows = read_csv_content(infile.read())
        for index, row in enumerate(rows):
            allocation_name=frappe.db.sql("select name from `tabLeave Allocation` where docstatus=1 and leave_type='Annual Leave - اجازة اعتيادية' and employee ='{0}' order by from_date desc limit 1".format('EMP000'+row[0]))
            
            doc = frappe.get_doc('Leave Allocation', allocation_name[0][0])
            doc.new_leaves_allocated = doc.new_leaves_allocated+flt(row[1])
            # doc.flags.ignore_validate = True
            # doc.save(ignore_permissions=True)

            print 'EMP00'+row[0],row[1]+'  **  '+str(doc.new_leaves_allocated)
...
def check_work_days():
    from frappe.utils.csvutils import read_csv_content
    from frappe.core.doctype.data_import.importer import upload
    with open('/home/frappe/frappe-bench/apps/client/client/check_work_days.csv', "r") as infile:  
        rows = read_csv_content(infile.read())
        for index, row in enumerate(rows):
            emp_work_days=frappe.db.sql("select work_days from `tabEmployee` where name='{0}'".format('EMP00'+row[0]))
            
            doc = frappe.get_doc('Employee', 'EMP00'+row[0])
            doc.work_days = row[1]
            doc.flags.ignore_validate = True
            doc.save(ignore_permissions=True)

            print 'EMP00'+row[0],row[1]+'  **  '+emp_work_days[0][0]



def increase_leave_balance():
    length=frappe.db.sql("select count(name) from `tabEmployee` where status!='left' and designation='فني' and name not in ('EMP0071','EMP0018') ")
    emp=frappe.db.sql("select name,work_days from `tabEmployee` where status!='left' and designation='فني' and name not in ('EMP0071','EMP0018') ")
    count=0
...
def is_supplier(party_name,row):
    supplier = frappe.db.sql("""select name from `tabSupplier` where (name REGEXP "^{0}")""".format(party_name))
    try:
        print(str(supplier[0][0]))
        print("Found Supplier:" )
        return str(supplier[0][0])
    except:
        return False

def is_customer(party_name,row):
    customer = frappe.db.sql("""select name from `tabCustomer` where (name REGEXP "^{0}") """.format(party_name))
    try:
        print(str(customer[0][0]))
        print("Found Customer:" )
        return str(customer[0][0])
    except:
        return False
...
def load_journal_entry_data():
    ...
    with open('/home/frappe/frappe-bench/apps/client/client/journal_new.csv', "r") as infile:  
        rows = read_csv_content(infile.read())
        ...
        for index, row in enumerate(rows):
            ...
            account_desc = row[8]
            dimension_1 = row[9]
            additional_desc = row[10]
            debit_sar = row[11]
            credit_sar = row[12]
            balance_sar = row[13]
            account_currency_str = row[14]
            fc_exchange_value = row[15]
            ...
            account_name = frappe.db.sql("""select name,account_currency from `tabAccount` 
                                            where (name REGEXP "^{0}") 
                                        """.format(account_number))
            cost_center = ""
            cost_center_list = frappe.db.sql("""select name from `tabCost Center` 
                                            where name like '{0}%' 
                                        """.format(cost_center_id))
            if(cost_center_list):
                print(account_desc)
                print(cost_center_list[0][0])
                cost_center = cost_center_list[0][0]
            ...
            if(stop):
                ...
                journal_entry.total_amount = journal_entry.total_amount
                # try:
                journal_entry.save()
                # except:
                #     pass                    
            ...
            #   break
            print(str(je_date))
            match = re.search(r'\d{2}-\d{2}-\d{4}', str(je_date))
            record_date = datetime.strptime(match.group(), '%d-%m-%Y').date()
            print(record_date)

            if account_desc and ((account_desc).encode("utf-8") == "Opening Balance"):
            ...
            if account_desc:
                record_description = account_desc
            
            print(party_name)
            emp_pattern = re.compile("^EMP.*[0-9]$",flags=re.IGNORECASE)
            if (dimension_1) and (emp_pattern.match(dimension_1)):
                party_type = "Employee"
                party = dimension_1
                emp = frappe.db.sql("select name from `tabEmployee` where name='{0}' ".format(dimension_1))
                print(emp)
                print("========%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%=====================================")
                ...
                if(party_name):
                    ...
                    for char in set(punctuation): 
                        party_name = party_name.replace(char,'\\'+char)

                    is_supplier_state = is_supplier(str(party_name),row)
                    is_customer_state = is_customer(str(party_name),row)
                    if(is_supplier_state and is_customer_state):
                        print("Supplier And Customer")
                        if(str(account_number == "20110103") or debit):
                            party_type = "Supplier"
                            # sup = frappe.db.sql("""select name from `tabSupplier` where (name REGEXP "^{0}") """.format(party_name),as_dict=1)
                            party = str(is_supplier_state)
                            print(is_supplier_state)

                        elif(credit):
                            party_type = "Customer"
                            # customer = frappe.db.sql("""select name from `tabCustomer` where (name REGEXP "^{0}")""".format(party_name),as_dict=1)
                            party = str(is_customer_state)
                            print(is_customer_state)

                    elif (is_supplier_state):
                        party_type = "Supplier"
                        # sup = frappe.db.sql("""select name from `tabSupplier` where (name REGEXP "^{0}") """.format(party_name),as_dict=1)
                        party = str(is_supplier_state)
                        print(is_supplier_state)
                
                    elif (is_customer_state):
                        party_type = "Customer"
            ...
            # account_doc = frappe.get_doc("Account",account_name[0][0])
            default_account_currency = account_name[0][1]
            print(default_account_currency)
            if account_currency_str == "EURO" and default_account_currency == "EUR":
                account_currency = "EUR"
                print("I did it!!!!!!!!!!!!!!!!!!!!")          
                # if((fc_exchange_value) != None):
                #     if(abs(float(fc_exchange_value))>0):
                if debit_sar:
                    # exchange_rate = abs(float(debit_sar)) / abs(float(fc_exchange_value))
                    # print(str(exchange_rate))
                    # print("((((((((((((((((((((((((((((((((((((()))))))))))))))))))))))))))))))))))))")
                    if not fc_exchange_value:
                        exchange_rate = 4.27
                        fc_exchange_value = abs(float(debit_sar)) / exchange_rate
                    else:
                        exchange_rate = abs(float(debit_sar)) / abs(float(fc_exchange_value))                        
                    debit_in_account_currency = (abs(float(fc_exchange_value)))
                    full_debit.append((abs(float(fc_exchange_value))) * exchange_rate)
                    print("14141414141414141414141414141414141414141414141414141414")
                    print((abs(float(fc_exchange_value))))
                if credit_sar:
                    # print(str(exchange_rate))
                    # print("((((((((((((((((((((((((((((((((((((()))))))))))))))))))))))))))))))))))))")
                    # exchange_rate = abs(float(credit_sar)) / abs(float(fc_exchange_value))
                    if not fc_exchange_value:
                        exchange_rate = 4.27                        
                        fc_exchange_value = abs(float(credit_sar)) / exchange_rate
                    else:
                        exchange_rate = abs(float(credit_sar)) / abs(float(fc_exchange_value))
                    credit_in_account_currency = (abs(float(fc_exchange_value)))
                    full_credit.append((abs(float(fc_exchange_value))) * exchange_rate)
                ...
                print(str(debit_sar))
                print(str(credit_sar))
                try:
                    if(debit_sar):
                        debit_in_account_currency = (abs(float(debit_sar)))
                        full_debit.append(debit_in_account_currency)
                    if(credit_sar):
                        credit_in_account_currency = (abs(float(credit_sar)))
        ...
        print("<<<<<<<<<<<<<<<Added All Journal Entries Available>>>>>>>>>>>>>>>")
